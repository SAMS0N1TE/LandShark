...
use_symbol (dsd_opts* opts, dsd_state* state, int symbol)
{
  ...
  state->umid = (((state->max) - state->center) * 1 / 2) + state->center;
  state->lmid = (((state->min) - state->center) * 1 / 2) + state->center;
  ...
  if ((state->synctype == 6) || (state->synctype == 14)|| (state->synctype == 18))
  ...
  else
    {
      ...
      #if 0
      {
        ...
        static int slicer_count = 0;
        if (state->lastsynctype == 0) {
          if (slicer_count < 12) {
            sys_log(0, "SL[%d] sym=%+5d lmid=%+5d center=%+5d umid=%+5d -> dibit=%d",
                    slicer_count, symbol, state->lmid, state->center, state->umid, dibit);
            slicer_count++;
          }
        } else {
          slicer_count = 0;
  ...
  /* Debug: log first 6 raw symbol values for NAC extraction */
  {
    ...
    if (state->lastsynctype == 0 || state->lastsynctype == 1) {
      if (slc < 6) {
        slv[slc++] = symbol;
        if (slc == 6) {
          sys_log(2, "SYM %d %d %d %d %d %d",
                  slv[0], slv[1], slv[2], slv[3], slv[4], slv[5]);
        }
      }
    }
    if (state->lastsynctype < 0) slc = 0;
  }
